...
const TextAreaWithIcon = forwardRef<HTMLTextAreaElement, TTextAreaWithIconProps>(({ icon, ...rest }, ref) => {
    return (
        <div className='p2p-text-area-with-icon'>
            <TextArea {...rest} className='p2p-text-area-with-icon__textarea' ref={ref} textSize='sm' />
            {<div className='p2p-text-area-with-icon__icon'>{icon}</div>}
        </div>
    );
});
