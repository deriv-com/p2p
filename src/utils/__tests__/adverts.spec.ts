...
describe('getLastOnlineLabel', () => {
    const lastOnlintTime = 1685446791;
    const isOnline = false;
    it('should return "Online" when lastOnlintTime is not passed and user is online', () => {
    ...
    it('should return "Seen n hours ago" when last seen is in hours', () => {
        ...
        expect(getLastOnlineLabel(isOnline, mockFn, lastOnlintTime)).toBe('Seen 3 hours ago');
    });
    it('should return "Seen 1 hour ago" when user was last seen 1 hour ago', () => {
        mockValue = moment('2023-05-30T16:49:38+04:00');
        (toMoment as jest.Mock).mockReturnValue(mockValue);
        expect(getLastOnlineLabel(isOnline, mockFn, lastOnlintTime)).toBe('Seen 1 hour ago');
    });
    ...
    it('should return "Seen n months ago" when last seen is in months', () => {
        ...
        (toMoment as jest.Mock).mockReturnValue(mockValue);
        expect(getLastOnlineLabel(isOnline, mockFn, lastOnlintTime)).toBe('Seen 5 months ago');
    });
